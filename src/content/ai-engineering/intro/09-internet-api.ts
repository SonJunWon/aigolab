...
솔직히 말해볼게요. **"API"** 라는 단어를 들으면 대부분 이렇게 생각합니다:

> "그건 개발자들한테 필요한 거 아니야? 나는 그냥 AI를 쓰면 되는데."

맞아요, 예전에는 그랬어요. 하지만 지금은 다릅니다.

**AI를 '쓰기만' 하는 사람**과 **AI를 '활용'하는 사람**의 차이가 바로 여기서 갈립니다.

| | API를 모르고 AI 사용 | API를 알고 AI 사용 |
|---|---|---|
| 할 수 있는 것 | ChatGPT 채팅창에서 대화 | AI를 내 업무에 자동으로 연결 |
| 반복 작업 | 매번 같은 질문을 직접 입력 | 한 번 설정하면 자동으로 실행 |
| 비용 | 월 구독료 (ChatGPT Plus 등) | 쓴 만큼만 내는 무료/저렴한 API |
| 확장성 | 채팅창 안에서만 | 엑셀, 블로그, 앱, 자동화까지 |
| 조합 | 하나의 AI만 사용 | 여러 AI를 파이프라인으로 연결 |

우리가 Ch07에서 배운 **쇼츠 파이프라인**을 기억하세요?
기획 AI → 대본 AI → 음성 AI → 이미지 AI → 음악 AI — 이 모든 연결이 **API** 로 이루어지는 거예요.

API를 이해하면:
- ChatGPT 월 구독료 없이 **무료 API 키로** AI를 사용할 수 있어요 (지금 AIGoLab에서 하고 있는 것처럼!)
- 매일 반복하는 작업을 **자동화**할 수 있어요
- 여러 AI를 **조합해서** 혼자서도 대단한 것을 만들 수 있어요
- 다른 사람이 만든 서비스를 **내 것처럼** 가져다 쓸 수 있어요

> 이번 챕터를 마치면 "API가 뭐야?"라고 묻는 친구에게
> "아, 그건 컴퓨터끼리 대화하는 방법이야!" 라고 자신 있게 설명할 수 있게 됩니다.
...
| 1 | API를 알면 뭐가 달라지는지 |
| 2 | 인터넷이 어떻게 작동하는지 |
| 3 | API = 식당의 메뉴판 (가장 쉬운 비유) |
| 4 | 우리가 이미 API를 쓰고 있었다! |
| 5 | API 키가 왜 필요한지 |`,
    },
    {
      type: "markdown",
...
| 배운 것 | 내용 |
|---------|------|
| 왜 API를 배우나? | AI를 '쓰기만' 하는 것과 '활용'하는 것의 차이 |
| 인터넷 작동 원리 | 내 컴퓨터 → 요청 → 서버 → 응답 → 내 컴퓨터 |
| API | 프로그램끼리 대화하는 방법 (식당의 메뉴판 같은 것) |
| 요청/응답 | 모든 인터넷 통신의 기본 패턴 |
| API 키 | API를 사용하기 위한 회원카드 (비밀번호처럼 관리!) |
| chat() | 우리가 이미 사용 중인 AI API! |

## 이 챕터의 핵심 메시지

> **여러분은 이미 API를 사용하고 있었습니다!**
>
> Ch01부터 지금까지 \`chat()\`으로 AI에게 질문할 때마다,
> 여러분의 브라우저에서 인터넷을 통해 AI 서버에 요청을 보내고
> 응답을 받아온 거예요. 그것이 API입니다.
>
> API를 이해하면, ChatGPT 채팅창에 갇히지 않고
> AI를 **내 업무에 맞게 자유롭게 연결**할 수 있습니다.
> 그것이 '쓰는 사람'과 '활용하는 사람'의 차이입니다.
