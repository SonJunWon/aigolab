...
### 1. 채팅 UI
- 화면 중앙에 채팅 영역 (최대 너비 640px)
- 상단: 챗봇 이름 + 아이콘
- 중앙: 메시지 말풍선 (사용자=오른쪽 파란, AI=왼쪽 회색)
- 하단: 입력창 + 전송 버튼 (Enter 키로도 전송)
- AI 응답 중 로딩 표시 (... 애니메이션)

### 2. 대화 로직
- 시스템 프롬프트: "너는 친절한 한국어 AI 어시스턴트야.
  짧고 따뜻하게 답해. 이모지를 적절히 사용해."
- 대화 히스토리를 배열로 관리
- 최대 20턴 유지 (초과 시 오래된 것 삭제)
- Gemini 2.5 Flash 모델 사용

### 3. API 키 관리
- 첫 실행 시 API 키 입력 모달 표시
- localStorage 에 저장
- 설정 버튼으로 키 변경/삭제 가능

### 4. 디자인
- 다크 모드 기본 (배경: #0f172a, 텍스트: #e2e8f0)
- 반응형 (모바일 대응)
- 부드러운 애니메이션
