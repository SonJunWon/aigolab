...
**해석**
- 말뭉치를 공백으로 자른 단어(토큰) **총 40개**, 이 중 **겹치지 않는 고유 단어는 17개**.
- 진짜 GPT 의 어휘는 **5만~20만 개** 규모. 우리는 17개짜리 미니 LLM 을 만든 셈.
- 토큰화 방식을 "공백" 대신 **BPE(Byte Pair Encoding)** 로 바꾸면 "오늘학교" 같은 붙은 단어도 분해됩니다 — 실제 GPT 가 쓰는 방식.
...
**해석 (핵심!)**
- 말뭉치에서 **"나는" 뒤에 "오늘" 이 2번, "어제" 가 2번** 나왔기 때문에 각 50%.
- 이것이 바로 **조건부 확률 \`P(다음 단어 | "나는")\`** — GPT 가 매 토큰마다 계산하는 바로 그 값.
- 진짜 GPT 는 어휘 전체(수만 개)에 대해 이 분포를 내뱉습니다. 우리는 두 개뿐이라 50/50.
...
\`\`\`
🎲 확률 샘플링으로 생성된 문장:
   나는 어제 공원에 갔다 나는 오늘 학교에
\`\`\`

**해석**
- "나는" → 50/50 확률에서 **"어제"** 당첨 → "어제" 뒤엔 "학교에/공원에" → **"공원에"** 당첨 → ...
- **같은 \`random.seed(42)\`** 면 매번 이 문장이 나옵니다 (재현성).
- \`seed\` 값을 바꾸면 **전혀 다른 문장** 이 생성됨. 이게 ChatGPT 답변이 매번 미묘하게 다른 이유와 동일한 원리.
...
| 한계 | 왜 문제인가 | 진짜 LLM 은? |
|------|-----------|-------------|
| **2단어(바이그램)만 본다** | "나는 오늘" 까진 보는데 그 앞은 무시 | 수천~수만 토큰의 문맥 |
| **말뭉치에 없으면 0%** | 처음 보는 조합은 생성 불가 | 유사한 패턴으로 일반화(임베딩) |
| **카운트 기반** | 의미를 모름 — 글자 조합만 셈 | **Self-Attention** 으로 의미·관계 학습 |
| **8문장으로 학습** | 똑같은 구조만 반복 생성 | 수조 개 토큰으로 학습 |

---

### 🧪 지금 시도해 볼 것

1. \`corpus\` 에 문장을 **한 줄 추가** — 그 단어 뒤 확률이 바뀌는지 확인.
2. \`seed\` 를 다른 짧은 단어(예: "그는", "우리는") 로 바꿔 실행.
3. \`random.seed(42)\` 의 숫자를 \`7\`, \`100\` 등으로 바꿔 실행 → 매번 다른 문장이 나옴.
4. \`corpus\` 에서 "나는 오늘" 관련 줄을 모두 지워 → "나는" 뒤에 "어제" 만 100% 나오는지 확인.

### 🏁 이 실습에서 배운 것
- ✅ **Next-Token Prediction**: LLM 의 근본 원리 — "다음 토큰의 확률 분포" 를 반복해서 뽑기
- ✅ **조건부 확률**: P(다음 | 지금까지) 라는 수학적 뼈대는 GPT-5 까지 동일
- ✅ **확률 샘플링**: "가장 높은 확률" 이 아닌 **확률대로 뽑기** 때문에 매번 다른 문장
- ✅ **규모의 차이**: 알고리즘 본질은 같고, **데이터·모델 크기 차이** 가 GPT 의 "지능" 을 만듦

> 💡 다음 섹션의 **Transformer** 는 이 바이그램을 "수천 토큰의 문맥" 으로 확장한 구조입니다. 원리는 같고 규모만 커진 것.`,
    },
    {
      type: "text",
