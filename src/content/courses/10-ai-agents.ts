...
`,
    ...
    },
    {
      ...
      content: `## 에이전트가 실제로 밟은 3가지 길

### 🧮 질문 1: "35 * 27 은 얼마야?"
\`\`\`
🧑  사용자: 35 * 27 은 얼마야?
🤖 [Step 1] 🧠 생각: 'calculator' 을 써야겠다
            📤 호출: calculator({"expression": "35 * 27"})
            📥 결과: {'ok': True, 'result': 945}
✅ [Step 2] 최종 답변: 답: 945
\`\`\`

**여기서 핵심은 "LLM이 직접 계산하지 않았다"** 는 것.
- Step 1: 질문에 \`*\` 가 있으니 **"계산기 도구가 필요"** 라고 판단 → \`expression\` 인자를 뽑아 도구 호출
- Step 2: 도구 결과(\`945\`) 를 관찰(observation) → **최종 답 모드** 로 전환

**왜 이게 중요한가?** LLM 단독으로는 큰 곱셈에서 종종 틀립니다. 계산기에 넘기면 **0% 오류** — **환각(hallucination) 제거**. 이것이 Tool Use 의 가장 대표적인 효용.

---

### 🌦️ 질문 2: "서울 날씨 알려줘"
\`\`\`
🤖 [Step 1] 🧠 생각: 'get_weather' 을 써야겠다
            📤 호출: get_weather({"city": "서울"})
            📥 결과: {'ok': True, 'weather': '흐림 12°C'}
✅ [Step 2] 최종 답변: 답: 흐림 12°C
\`\`\`

- 같은 루프 구조, **다른 도구 선택**.
- "날씨" 키워드 + "서울" 매칭 → \`get_weather\` 로 라우팅.
- **"지식 컷오프 극복"** 의 미니어처 — LLM 이 학습한 2024년 데이터가 아니라, **지금 이 순간** 의 도구 응답.

---

### ⚠️ 질문 3: "오늘 코스피 지수 알려줘"
\`\`\`
✅ [Step 1] 최종 답변: 제가 이 질문을 위한 도구를 갖고 있지 않아요.
\`\`\`

**해석 (중요)**
- 도구 목록에 **주식 조회 도구가 없음** → 에이전트가 **가짜 답을 지어내지 않고** 솔직히 실패.
- 이것이 **"Graceful degradation"** — 좋은 에이전트는 **모르는 걸 안다고 인정**.
- 진짜 LLM 은 여기서 \`get_stock_price\` 같은 도구를 **요구** 하는 식의 응답을 줄 수도 있음.
...
Reason  →  Act      →  Observe  →  Reason  →  ...
 (생각)    (도구 호출)   (결과 관찰)    (다음 생각)

 "곱셈이네"  calculator   945         "이제 답할 수 있다"
              호출        받음           → final
\`\`\`

- 이 코드에서 \`fake_llm\` 이 하는 역할은 실제 Claude/GPT 가 **JSON 형식의 tool_call** 로 하는 일과 동일.
- \`history\` 배열이 **에이전트의 "단기 기억"** — 지금까지 뭘 시도했는지 보관.
- \`max_steps = 5\` 는 **무한 루프 방지** (도구가 실패를 반복하면 강제 종료).

---

### ⚠️ 이 구현의 한계

| 한계 | 실제 Claude / GPT 에이전트는? |
|------|-------------------------------|
| **키워드 매칭 LLM** | JSON 스키마를 이해하는 진짜 LLM 이 도구를 **의미적으로** 선택 |
| **한 번에 한 도구** | 여러 도구를 **순차/병렬** 로 연쇄 사용 가능 |
| **고정 날씨 DB** | 실제 API 호출 (OpenWeatherMap 등) |
| **에러 복구 없음** | 도구 실패 시 **대안 도구** 를 시도하거나 사용자에게 되물음 |
| **\`eval()\` 사용** | 프로덕션은 샌드박스·타임아웃·입력 검증 필수 |

---

### 🧪 지금 시도해 볼 것

1. **새 도구 추가**: \`get_time\` 함수를 정의하고 \`TOOLS\` 에 등록 + \`fake_llm\` 에 키워드 분기("시간") 추가. \`agent("지금 몇 시야?")\` 로 테스트.

2. **복합 질문**: 현재 \`fake_llm\` 은 한 번에 한 도구만 호출합니다. "서울 날씨랑 35+27 동시에 알려줘" 같은 질문을 처리하려면 어떻게 수정해야 할지 생각해 보기.

3. **새 도시 추가**: \`get_weather\` 의 \`db\` 딕셔너리에 "대전" 을 넣고 \`fake_llm\` 의 도시 리스트에도 추가.

4. **실패 시나리오 만들기**: \`agent("나누기 좀: 10 / 0")\` → 계산기 도구의 \`ok: False\` 분기가 어떻게 처리되는지 확인.

---

### 🏁 이 실습에서 배운 것
- ✅ **ReAct 루프 = AI 에이전트의 심장**: Claude Code, Cursor, ChatGPT Agents 모두 이 구조
- ✅ **Tool Use = 환각 제거의 핵심**: 계산·검색·DB 조회를 **코드에 위임**
- ✅ **"말하는 AI" → "일하는 AI"**: 파일 수정, API 호출, 스크립트 실행까지 확장
- ✅ **솔직한 실패**: 좋은 에이전트는 **모르는 걸 지어내지 않음** (graceful fail)

> 💡 여러분이 막 구현한 이 30줄이, GitHub Copilot / Claude Code 의 가장 안쪽 엔진과 개념상 **동일** 합니다. 남은 건 도구의 수와 LLM 의 크기뿐.`,
    },
    {
      type: "text",
