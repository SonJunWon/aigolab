...
- **강의 정리 — 녹음 원본 다운로드**
  - 오디오 플레이어에 '⬇ 다운로드' 버튼: 보관 원본을 재인코딩 없이 무손실로 내보냄.
  - 청크 1개(≤5분)는 `.webm` 단일 파일, 여러 개면 무압축 ZIP 하나로 (파트 파일명에 번호·시작 오프셋 표기, 예: `제목-03-010m.webm`).
  - ZIP 은 의존성 없이 직접 생성(`audioExport.ts`) — UTF-8 파일명 플래그로 한글 안전, 타임스탬프 = 녹음 시각. unzip/ditto/Python zipfile 검증 완료.
...
- **강의 정리 — 녹음 다시 듣기 플레이어 (배속 지원)**
  - 노트 상세에 오디오 플레이어: IndexedDB에 보관된 5분 청크(webm)를 하나의 타임라인으로 이어서 재생 — 청크 경계 자동 연결, 슬라이더 탐색, ⏪10초/10초⏩ 이동.
  - 배속 재생: 0.75x / 1x / 1.25x / 1.5x / 1.75x / 2x — 청크 전환에도 배속 유지.
  - 북마크 점프 칩: 녹음 중 찍은 🔖 시점으로 바로 이동.
  - '오디오 삭제' 버튼: 노트·전사·정리본은 유지한 채 오디오만 폐기(보관 용량 MB 표시).
  - 원본이 없는 노트(폐기됨)는 안내 문구 표시.

### Changed

- **오디오 원본 보관 기본값 ON** — 기존 '정리 후 폐기'에서 변경. 다시 듣기가 기본 동작이 되도록 함 (32kbps 모노 ≈ 시간당 14MB, 로컬 IndexedDB 전용·클라우드 미업로드). 녹음 폼 체크박스로 해제 가능.
...
- **강의 정리 — 노트 PDF 저장**
  - 노트 상세에 '📄 PDF 저장' 버튼: 인쇄 렌더 방식(숨김 iframe + @page CSS)으로 브라우저 'PDF로 저장' 호출 — 의존성 0, 한글 벡터 텍스트(선택·검색 가능).
  - 구성: 제목·메타(강의/회차/출처/태그) → 한줄요약·개념 카드·타임라인·북마크·용어·액션·인용 → 부록 원문 전사(새 페이지) → 개인 보관 고지 푸터.
  - 정리본 없는 노트(전사만)도 PDF 생성 가능. 제안 파일명 = 노트 제목.
...
- **강의 정리 — 강의 상세에서 세션 추가/빼기**
  - 📥 '미분류에서 추가': 미분류 녹음을 체크박스로 골라 현재 강의의 세션으로 일괄 편입.
  - 세션별 '빼기' 버튼: 강의에서 분리해 미분류로 (노트 비파괴·가역 — 확인창 없이 즉시).
  - 세션 변경 시 통합 정리 '오래됨' 배지 연동 유지.
...
- **강의 정리 — 예약 녹음 (시각 지정 자동 시작)**
  - 녹음 시작 화면에서 `datetime-local`로 시각 지정 → '⏰ 예약 녹음'. 도달 시 현재 폼 설정(강의·회차·제목·원본 보관)으로 자동 시작
  - 예약 시 마이크 권한을 **선확보**(요청 후 즉시 해제) — 시작 시각에 권한 프롬프트로 멈추지 않음
  - 전역 스토어 타이머(5초 주기 검사 — 백그라운드 탭 스로틀 대응): SPA 내 다른 페이지에 있어도 실행. localStorage 저장으로 새로고침 후 관리자 페이지 재방문 시 예약 자동 복원
  - 안전장치: 예약 시각을 10분 초과해 지나침(잠자기 등) → 뒤늦은 마이크 자동 켜짐 대신 취소+사유 표시. 수동 시작 시 대기 예약 자동 소비. 카운트다운·예약 취소 UI
...
- **강의 정리 v2 M2 — 자료 관리 + 통합 정리본**
  - 📎 자료 3종: 파일(파일당 100MB, IndexedDB Blob, 열기/삭제, 합계 용량 표시) · 링크 · 메모 — 강의 상세에서 관리.
  - 🧾 통합 정리본: 세션 정리본들의 재종합(summarizeLectureOverview, outline=회차 흐름). 세션 변경 시 '오래됨' 배지 + 수동 재생성.
  - 정리본 렌더러 SummaryBlock 공용화 (노트 상세·강의 상세 공유).
...
- **강의 정리 v2 M1 — 강의 묶기 (2계층 구조)**
  - 📚 강의(Lecture) 상위 단위 신설: 총괄 제목·강연자(문자열)·설명·태그. IndexedDB v2(lectures·materials 스토어).
  - 뭉치기 3경로: ① 녹음 폼 강의 선택(＋새 강의 즉석 생성, 회차 라벨 자유 텍스트) ② 목록 미분류 노트 다중 선택 → 강의로 묶기 ③ 노트 상세 소속/회차 편집.
  - 강의 상세 화면: 헤더 인라인 편집 · 세션 노트 목록(회차 배지) · '＋ 이 강의로 새 녹음'(회차 자동 제안) · 강의 삭제(노트는 미분류로 — 비파괴).
  - 하위 호환: 기존 노트는 전부 미분류로 동작 불변. 기획: 02-강의묶음-자료관리-기획.md (M2=자료·통합 정리, M3=분할·Supabase).
...
- 공지사항: 「AI 에이전트 엔지니어링 3부작 완결 — 고급1·2·3 총 72강」 고정 공지 추가 (n010). (f33e08d 커밋 메시지의 v4.47.1 표기는 오기 — 동시 릴리즈로 v4.48.1로 정정)
...
- **강의 정리 — 중단된 녹음 세션 복구 UI** (`OrphanRecoverySection`)
  - 새로고침·dev HMR 리로드·크래시로 종료 처리 없이 끊긴 녹음을 목록 화면 상단에서 감지 (IndexedDB 청크 中 활성 녹음·저장된 노트 제외)
  - '♻️ 노트로 복구': 기존 파이프라인 재사용 (`transcribeSession` 진행률 → `summarizeTranscript` → `saveNote`) — AI 정리 실패 시 전사만 저장 후 상세에서 재시도
  - 복구 노트는 `keepAudio: true`로 원본 오디오 보존 (노트 삭제 시 함께 폐기) · 시작 시각은 sessionId 타임스탬프에서 복원 · '폐기' 버튼으로 청크 정리 가능
  - 한계: 북마크(메모리 유실)·마지막 5분 미만 미완 구간은 복구 불가
...
- **고급3 「평가·운영」 시리즈 C '운영 만들기' 10강 공개 — 3중 시리즈 24강 완성 · AI 에이전트 엔지니어링 3부작(72강) 대장정 완결** 🎉 (PRO, 개발자)
  - **C1~C6 운영 계층**: C1 계면 원칙(run 레코드·시간 주입) · C2 텔레메트리(집계→비교→판정, 🟢는 침묵) · C3 평가 러너(LLM 판정자 3대 편향·판정자 골든) · C4 현장 신호(수정 diff·재실행 자동 감지·불일치 경보) · C5 실패 분류기(규칙 8할+LLM 2할·족보 승격) · C6 회귀 게이트(N회 평균·노이즈 판정·케이스별 우선)
  - **C7~C10 실전 아크 '코드리뷰 회사 운영 30일'**: C7 카나리 라우터(해시 분할·배포 없는 롤백) · C8 비용 거버너(우아한 저하 3모드·정직 라벨·2단 알람) · C9 사고의 날(드리프트 감지→분류→재현→임시 다이얼→포스트모템 자동 초안→박제) · C10 운영 센터 총조립 + 캡스톤 + 확장 연구(자가 운영·운영의 운영)
  - 전 강 llm-code 실행 셀 + 연구 노트. 트랙 ~12시간 완성. 용어집 '텔레메트리' 추가.
...
- **고급3 「평가·운영」 시리즈 B '운영 지휘하기' 8강 공개** (PRO)
  - B1 운행일지 읽기(세 줄 읽기·4대 패턴·무소식은 최대 경보) · B2 3숫자 계기판(팀별 번역·기준선·노랑/빨강) · B3 주간 15분 리뷰(운영 시간 예산: 매일 10초·매주 15분·매달 1시간)
  - B4 현장 신호(수정·재요청·무시 10초 양식, 채점표도 기른다) · B5 개선 대장(근거 없으면 기각·회귀 5건 전부·손 카나리 3형태) · B6 사고 플레이북(3등급·첫 행동 카드·감지 지연 지표)
  - B7 운영 장부(사용 횟수 기준 수지·유지/축소/폐지·분기 좀비 사냥) · B8 캡스톤(플레이북 7부품 조립 + 첫 리뷰 실행 — 매뉴얼과 플레이북 한 쌍 완성)
  - 트랙 ~5.5시간으로 확대. FREE 미등록 → 자동 PRO.
...
- **고급3 「평가·운영」 트랙 신설 + 시리즈 A '운영의 탄생' 6강 — 전면 무료** (AI 에이전트 엔지니어링 3부작 완결편)
  - 척추 개념 "달리게 만드는 것과 계속 달리게 하는 것은 다른 기술이다" — 출시는 관측의 시작. 운영 루프 5국면(관측·평가·진단·개선·통제).
  - A1 출시는 시작이다(드리프트 3범인) · A2 계기판(3숫자·추이·행동 연결) · A3 성적표의 계보(골든 vs 암묵 신호, 불일치가 신호) · A4 포스트모템(비난 없음·구조 방지·골든 박제) · A5 개선의 규율(회귀·카나리·롤백) · A6 운영 실패학(계기판 중독·감상 운영·알람 피로·좀비·운영자 부재)
  - Track 타입 advanced3, 트랙 카드(🎛 teal), 용어집 3종(드리프트·카나리·포스트모템). 시리즈 A는 FREE.
...
- **강의 정리 — 녹음 세션이 페이지 이동에도 지속되도록 전역화**
  - 기존: 녹음 상태가 컴포넌트 state 라서 탭·페이지 이동 시 녹음 화면 소실 + 녹음기 참조 유실(유령 녹음).
  - 수정: `lectureRecordingStore`(zustand) 신설 — 녹음기 핸들·라이브 STT 파이프라인·구간 노트·종료 파이프라인을 전역 싱글턴으로 이동.
  - 결과: SPA 내 어디로 이동해도 녹음·실시간 변환 계속, /admin 재진입 시 강의 정리 탭 자동 선택 + 녹음 화면 복원, 탭 라벨 🔴 표시, 종료 파이프라인도 이동 중 계속 진행.
  - beforeunload 경고를 녹음 수명주기(스토어)에 결합. 새로고침·탭 닫기는 브라우저 한계로 여전히 중단(경고창으로 방지, 청크는 보존).
...
- **강의 정리 — 실시간 구간 노트 (라이브 노트)**
  - 녹음 중 5분 청크가 저장되는 즉시 STT → 구간 불릿 요약(2~4개)을 화면에 실시간 누적 표시 (🎧 변환 중 → ✍️ 정리 중 → ✅).
  - 상단 컨트롤 바: 경과 시간 · 다음 구간 정리 카운트다운 · 북마크 · 종료.
  - recorder `onChunk` 콜백 + `transcribeChunkBlob` + `quickChunkSummary` 신설. 청크 처리는 직렬 큐.

### Changed

- 종료 파이프라인 단축 — 라이브에서 이미 변환된 전사를 재사용해 최종 정리만 수행(전체 재변환 제거). 라이브 변환 전멸 시(키 부재 등) 기존 일괄 변환으로 자동 폴백, 실패 구간은 전사에 명시.
...
- **고급2 「멀티에이전트 시스템」 시리즈 C '팀 만들기' 10강 공개 — 3중 시리즈 24강 완성** 🎉 (PRO, 개발자)
  - **C1~C6 런타임 확장**: C1 spawn의 도구 승격(agent-as-tool) · C2 역할 레지스트리(화이트리스트·모델 믹스) · C3 파이프라인·팬아웃(동시성 캡·부분 실패 격리) · C4 슈퍼바이저 루프(폭주 가드 3종) · C5 산출물 계약(스키마 강제·반려 재시도·요약+위치) · C6 트레이스 트리(계보·비용 회계·실패 재현)
  - **C7~C10 실전 아크 '자동 코드리뷰 회사'**: C7 설계 회의(3안 비교·Finding/Verdict 계약·5대 축 검토) · C8 구현(결함 심은 PR에서 파인더 팬아웃→적대적 검증단 2/3 다수결이 오탐을 걸러내는 하이라이트) · C9 강화(골든 PR 성적표·예산 제한기·최소 권한 감사) · C10 리서치 뉴스룸 일반화(골격 재사용성 증명) + 캡스톤 + 확장 연구(A2A·컴퓨터 사용·시장 조율·고급3 예고)
  - 전 강 llm-code 실행 셀(chat() 프리미티브) + 연구 노트. 트랙 ~12시간 완성. 고급1 C(미니 하네스)의 후속 — 스타터 킷 내장으로 단독 수강도 가능.
...
- **고급2 「멀티에이전트 시스템」 시리즈 B '팀 지휘하기' 8강 공개** (PRO)
  - B1 팀 기능 해부(스폰·격리·회수, 비밀번호 실험) · B2 직무기술서 쓰기(역할·범위·산출물 + 견본의 힘) · B3 파이프라인 위임(인수인계 계약·추가 금지 조항)
  - B4 병렬 위임과 취합(독립 판정 2문항·취합자 3대 임무·병렬의 청구서) · B5 검증 팀(적대적 검토자 템플릿·검증 3단 수위·왕복 대조표)
  - B6 자동 파이프라인(침묵 금지 보고·상한 3종·아침 브리핑 팀) · B7 팀의 성적표(단일 vs 팀 맞대결·가린 채점·팀 골든 케이스) · B8 캡스톤(팀 운영 매뉴얼 6부품 조립 + 실전 완주·회고)
  - 각 강 ai-try 산출물이 B8 매뉴얼의 부품이 되는 구조(고급1 B 관행). 트랙 ~5.5시간으로 확대. FREE 미등록 → 자동 PRO.
...
- **고급1 「하네스 엔지니어링」 시리즈 C '하네스 만들기' 10강 공개 — 3중 시리즈 24강 완성** 🎉 (PRO)
  - C1 100줄의 에이전트(최소 하네스) · C2 도구 설계 · C3 지침 아키텍처 · C4 컨텍스트 공학 · C5 파일 메모리
  - C6 브레이크(샌드박스·권한·HITL) · C7 서브에이전트 · C8 살아있는 런타임 · C9 평가 하네스 · C10 캡스톤(미니 Claude Code 조립)
  - chat() 원시 호출로 루프부터 평가까지 직접 구현. 트랙 ~13시간으로 확대.
- **고급2 「멀티에이전트 시스템」 트랙 신설 + 시리즈 A '팀의 탄생' 6강 — 전면 무료**
  - 척추 개념 "말 한 마리는 짐을 끌고, 마차단은 도시를 옮긴다" — 멀티에이전트 = 조직 설계. 5대 설계 축(분업·역할·지휘·전달·신뢰).
  - A1 천재 한 명의 한계(컨텍스트 물리학) · A2 조직도 그리기(파이프라인 vs 전문화 + 직무기술서 3요소) · A3 지휘 패턴 동물원(라우터·슈퍼바이저·병렬·판정단)
  - A4 전달의 기술(전화 게임 → 산출물 계약) · A5 검증하는 팀(적대적 검증·판정단 다수결) · A6 멀티에이전트 실패학(4대 골절 + '팀이 필요한가' 3문항)
  - 균형 원칙 "멀티는 답이 아니라 비용" 전 강 반복. B '팀 지휘하기'(8강)·C '팀 만들기'(10강, 실전: 자동 코드리뷰 회사) 집필 예정.
  - Track 타입 advanced2, 트랙 카드(🏇 indigo), 용어집 4종(오케스트레이션·산출물 계약·적대적 검증·컨텍스트) 추가. 시리즈 A는 FREE 등록.
...
- **고급1 「하네스 엔지니어링」 시리즈 B '하네스 다루기' 8강 공개** (PRO)
  - 대상: AI를 조금 다루는 비개발자 — 코드 없이 설정·지침으로 기성 하네스를 조련.
  - B1 해부 체크리스트 · B2 지침 파일(3원칙) · B3 도구 연결 3문답·큐레이션 · B4 컨텍스트 연출(요약 이어달리기·메모리 청소) · B5 권한 시나리오 10 판단 훈련 · B6 스킬·훅·예약 자동화 · B7 골든 케이스 5·회귀 평가 · B8 캡스톤 '업무 하네스 설계서'.
  - 각 강의 실습 산출물이 B8 설계서의 부품이 되는 누적 구조. order 11~18 (시리즈 A 6강에 이어짐).
  - 참고: B1~B3 파일은 v4.36.0에 선반영됨(index 미등록 상태) — 본 릴리즈에서 8강 전체 등록·노출.
- 용어집: '회귀 평가', '골든 케이스' 추가.

### Changed

- 고급1 트랙 카드: 배지 '무료'→'A 무료', 시리즈 A+B 구성 설명, 분량 ~5.5시간(estimatedHours 6).
...
- **라이트 모드 점검(UAT) 및 오버라이드 버그 수정**
  - `hover:` 전용 클래스가 패턴 매칭(`[class*="…"]`)에 걸려 **상시 적용**되던 버그 수정 — AI 앱 개발 등에서 카드 보더가 항상 주황으로 보이던 문제. accent 보더/텍스트/배경·패널 배경을 `:not([class*="hover:…"])` 가드 + `:hover` 전용 규칙으로 분리.
  - 라이트 모드 primary 보더 추가(네온 그린 → 진한 그린 rgba(5,150,105,0.45), hover #059669).
  - 준비 중 점선(`border-brand-line/40`)이 라이트에서 불투명 강제되던 것 → rgba 0.4 유지.
...
- **흰 구획선 규칙 내부 페이지 확산** (v4.35.0 리디자인 후속)
  - AI 앱 개발·AI 엔지니어링 트랙·워크샵 목록/상세·학습 진도·마이·공지·FAQ·프로젝트 — 카드 컨테이너 전부 라운딩 제거 + `brand.line` 흰 라인 박스로 통일.
  - 구분 원칙: 구조 컨테이너=흰 라인 플랫 / 컨트롤(검색·필터 칩)=소프트 보더 유지 / 준비 중·빈 상태=점선 흐림 / Phase·파트 시맨틱 컬러 보더=색 유지.
- **설명 텍스트 가독성 상향**
  - 전역 기본 폰트 14px → 15px (md-prose 레슨 본문 포함).
  - 페이지 히어로 소개문 text-sm → text-base/lg, 카드 설명 text-sm → text-base, 쇼케이스·강의 부제 text-xs → text-sm. 모노 마이크로 라벨은 디자인 요소로 유지.
...
- **전면 리디자인 — DecideAI(decideai.xyz) 모티브 에디토리얼 다크** (보라 'Claude 틱' 탈피)
  - **팔레트**: 배경 `#050505` 블랙 + 네온 그린 `#73ffb9`(primary) + 오렌지레드 `#ff6338`(accent). 신규 토큰 `brand.line`(`#f4f6f6`) — 흰색 구조 구획선.
  - **타이포**: Pretendard Variable(본문) + JetBrains Mono(라벨·숫자) CDN 로드. `.mono-label`·티커·라인필·rise 유틸 추가.
  - **홈**: 좌측 정렬 거대 타이포 히어로 + 브랜드 티커 + 넘버링 섹션(001.) + 흰 라인 공유 그리드(학습 코너 3셀·통계 4셀·CTA/지원 2셀).
  - **코스 페이지**(/courses·그룹 상세): 넘버링 행 + 흰 라인 박스. 이용 가능=솔리드 박스+그린 화살표 / 준비 중=점선+흐림.
  - **어포던스 규칙**: 클릭 가능=흰 라인 박스+상시 그린 화살표+hover 패널 / 클릭 불가=화살표·hover 없음.
  - **NavBar/Footer**: 플랫화(이모지 제거, 그린 도트 로고, 흰 구획선, 모노 카피라이트).
  - **레슨 본문(md-prose)**: 인라인 코드·링크·핵심 카드·용어 툴팁 보라 → 그린/무채색. 수료증 SVG·로그인 로고 새 브랜드로.
  - 오렌지→시안 그라데이션 CTA 전부 솔리드 그린+검정 글자로 플랫화(챗봇·로그인 유도·워크샵·진도 바). 그린 배경 위 흰 글자 → 검정 글자 전역 수정.
  - 라이트 모드: 네온 그린→진한 그린, 흰 구획선→진한 라인 자동 반전. Phase 1~8 시맨틱 9색·콜랩(코딩 실습) 팔레트는 유지.
...
- **고급1 「하네스 엔지니어링」 시리즈 A 6강 신설 — 전면 무료 공개** (트랙 유입용, B·C 시리즈는 PRO 예정)
  - A1 같은 뇌, 다른 몸 — 하네스란 무엇인가 · A2 심장 박동 — 에이전트 루프 · A3 도구 — 손과 발
  - A4 좁은 책상 — 컨텍스트와 기억(책상·수첩 모델) · A5 안전벨트 — 권한과 가드레일 · A6 하네스 실패학(고장 부위 진단)
  - 각 강에 AI-직접-질문·사례 바로 실행(ai-try)·이해도 퀴즈 포함. 트랙 등록(index·languages·tier·glossary 42개 용어) 완료.
  - `Track` 타입에 `"advanced1"` 추가.
...
- **코스 2 「지식 증류」 모듈 3 추가 (14~18강)** — SFT 일반(FFT vs PEFT). 전체 34강 중 누적 18강 공개.
  - 14 SFT 왜 필요(프롬프트·RAG·SFT 경계) · 15 FFT(전체 파인튜닝 메모리 비용) · 16 PEFT & LoRA(저랭크 어댑터, 학습 파라미터 3% 검증) · 17 QLoRA(4비트 압축·무료 Colab 메모리 계산) · 18 TRL & Unsloth 실습(외부 Colab #1, Unsloth 공식 노트북 링크).
  - LoRA 수학 토이 numpy 검증. 실제 GPU 훈련(18강)은 정직 원칙대로 무료 Colab으로 안내.
...
- **코스 2 「지식 증류」 모듈 2 추가 (9~13강)** — MoE & SFT 개념. 전체 34강 계획 중 모듈 2(누적 13강 공개).
  - 9 MoE 기본(전문가·라우터·top-K·총vs활성) · 10 라우팅과 부하 균형(쏠림→aux loss) · 11 실전 MoE: DeepSeek(671B/37B·공유전문가·MLA·스펙 계산기) · 12 SFT 개념(사전학습→SFT→정렬, 의도 파악) · 13 파인튜닝 일반(습관 vs 지식, 파인튜닝 vs RAG).
  - 브라우저 Pyodide 실습(미니 MoE forward·부하 균형 before/after·모델 스펙 계산기·before/after 예시) — 로컬 numpy 검증. 매 강 퀴즈.
...
- **MCP 04강에 실전 시나리오 섹션 추가** — 음성 회의록 앱 NSTT_Notion(바이브 코딩 제작)으로
  회의를 받아 → 결과를 클로드 코드(노션 MCP)에 붙여넣어 → 노션에 회의록 DB·페이지가 자동
  생성되는 전체 흐름을 스크린샷 5장(앱 설정·내보내기·MCP 호출·노션 DB·완성 페이지)+설명으로
  구성. 이미지 public/lessons/mcp-notion/scenario-1~5.
...
- **MCP 04강 Notion 재작성/확장** (`mcp-special/04-notion`, ~10분→20분) — Notion 공식 MCP
  문서(2026-06 확인) 기준으로 깊이 보강:
  - **연결 상세**: 호스티드 주소 `mcp.notion.com/mcp`(Streamable HTTP)·`/sse`(레거시),
    OAuth 전용, 앱별 방법(Claude Desktop·ChatGPT 원클릭 / Cursor·VS Code 등 .mcp.json /
    mcp-remote 브리지).
  - **지원 범위**: 읽기+쓰기 ~18개 도구 표(검색·조회·댓글·DB질의[유료] / 페이지·DB·뷰 생성·수정).
  - **한계와 대책 표**: 레이트리밋(180/분·검색 30/분), relation 25개 캡(조용히 잘림),
    OAuth 사람 동석→무인 자동화 부적합, 유료 플랜 도구, 베어러 미지원 + 각 대책.
  - **MCP 제공 방식의 변화 흐름**: SSE→Streamable HTTP, 검색→쓰기 확장, 로컬→원격
    호스티드+OAuth(로컬 오픈소스 서버 유지보수 중단), 지원 앱 증가.
  - ai-try 2→3개(한계 대책 설계 추가), 퀴즈 4→5문항(지원범위·25캡·변화흐름).
- 카탈로그(`02-MCP-도구별-적용예시-카탈로그.md`)에 공식 문서 기반 검증 갱신 기록(로컬 서버
  유지보수 중단 등 보정).
...
MCP 특별강의 **도구별 적용편 시작 — 04강 Notion(직장인용 따라하기)** 신설.

### Added

- **MCP 04강 "[적용] 노션을 AI에 연결해 업무에 쓰기"** (`mcp-special/04-notion`) — 카탈로그
  Notion 항목 기반, 일반 직장인/회사원이 **코딩 없이 따라 하며 바로 업무에 적용**하는 실전편:
  - 왜 노션+AI(직장인 고통) · 읽기/행동 표 · OAuth 연결 4단계(코딩 없음) · 처음이라면
    가이드(개인 노션 먼저·무료 시작·로그인 비유) · 회의록→노션 정리/주간보고 사례 ·
    안전 수칙(읽기 먼저·특정 DB 한정·회사 정책 확인) · 미션 · 퀴즈 4문항.
  - 실습 ai-try 2개: 노션 페이지용 회의록 정리 / 주간 보고서 초안(노션 없이도 연습 가능).
  - MCP 생태계 변동성 대비 "최신은 공식 문서 확인" 병기.
- 직장인 '따라하기 충분성' 시뮬레이션(페르소나 워크스루)으로 연결 단계 진입 마찰 보강.
...
- MCP 특별강의 3강의 🎬 샘플 사례마다 **즉시 실행 ai-try 박스 추가**(강당 1→2개, 총 6개) —
  서버 고르기 / 읽기·쓰기 도구 나누기 / 행동마다 가드 붙이기. 플랫폼 전반의 친화 강의
  일관성(강당 ai-try 2개)에 맞춤.

---

## [4.22.0] - 2026-06-02

**MCP 특별강의(가교 특강) 3강 신설** — 중급1(지식 트윈)↔중급2(에이전틱)를 잇는 표준 연결.

### Added

- **MCP 특별강의 트랙 신설** (`mcp-special`, 3강) — 기획(AI앱개발/MCP-특별강의/) 기반,
  친화 스타일(카드·USB-C 비유·🎬사례·ai-try·퀴즈):
  - 01 MCP란 무엇이고 왜 필요한가 (AI용 USB-C, Resources/Tools/Prompts 3요소, 서버↔클라이언트)
  - 02 내 트윈을 MCP로 노출하기 (Resource/Tool 노출, 도구 스펙, 읽기→쓰기 안전 순서)
  - 03 에이전트가 MCP 도구를 소비하기 (발견→호출 흐름, 3대 가드: 스코핑·HITL·인젝션 — 중급2로의 다리)
- 트랙 등록 5곳 연결: `Track` 타입, `languages.ts`, `content/index.ts` CURRICULA,
  `mcp-special/` 콘텐츠+index, **AI 엔지니어링 트랙 페이지**(`/ai-dev/track`)의 'MCP 특별강의'
  카드를 '준비 중' → **활성 링크(NEW)** 로 전환.

### Notes

- PRO 게이팅: 다른 ai-engineering 레슨과 동일(무료 목록 외 → PRO). 학습 흐름:
  입문 → 중급1 → **MCP(연결)** → 중급2 → 워크샵.

---

## [4.21.0] - 2026-06-02

AI 입문준비과정 **14강(Ch00~13)을 친화 하이브리드 버전으로 완성** — 코드 유지 + 친화 톤.

### Added

- **친화 하이브리드 입문준비 14강 신설** (`friendly-intro/00~13`) — 다른 트랙과 달리
  **코드를 유지**하는 하이브리드: 원본(`intro/NN`)의 코드 셀(llm-code, 실행 실습)과
  퀴즈를 **그대로 import 재사용**하고, 설명만 비개발자 친화 카드(🃏·비유·🎯)로 감쌈.
  - 00 시작하기 / 01 첫 대화 / 02 글쓰기 / 03 그림 그리기 / 04 원리와 한계
  / 05 프롬프트 / 06 마크다운 / 07 AI 종류 / 08 파일 확장자 / 09 인터넷·API
  / 10 코딩 기초 / 11 개발 도구 / 12 바이브 코딩 / 13 다음 단계

### Changed

- **입문준비(intro) 트랙 Ch00~13 친화 하이브리드로 대체** — `intro/index.ts` 가
  `friendly-intro/NN` 을 가져옴(기존 id·order 유지). 친화 파일은 코드판 `intro/NN-*.ts`
  의 코드 셀을 직접 import 재사용하므로 코드는 그대로 보존·실행됨(순환 import 없음).
  **이로써 AI엔지니어링 전체(입문준비 14 + 입문 12 + 중급1·8 + 중급2·7 = 41강) 친화 완성.**

---

## [4.20.0] - 2026-06-02

AI 엔지니어링 **중급2 "에이전틱" 7강 전체를 친화 버전으로 완성**.

### Added

- **친화 중급2 7강 신설** (`friendly-intermediate2/01~07`) — "트윈에 손발을 단다(행동하는
  에이전트)" 척추 개념을 비개발자용 카드·비유·🎬사례·즉시실행 ai-try 박스·퀴즈로 재구성:
  - 01 안 죽는 일꾼 AI(견고한 에이전트) / 02 여러 일꾼 지휘(오케스트레이션)
  - 03 긴 대화에도 안 까먹기(메모리) / 04 행동하되 위험한 건 사람이(HITL·MCP)
  - 05 느낌 말고 숫자로 평가(평가·관측) / 06 싸고 빠르고 안전하게(비용·보안·인젝션)
  - 07 전부 합친 진짜 일하는 AI(Capstone 배포형 에이전트)
  - 각 강 ai-try 2개씩(총 14개).

### Changed

- **중급2(intermediate2) 트랙 01~07 전부 친화 버전으로 대체** — `intermediate2/index.ts`
  가 `asIntermediate2()` 매핑(기존 id·order·PRO 게이팅 유지). 코드판 `NN-*.ts` 는 파일로
  보존(번들 제외). **이제 입문 12강 + 중급1 8강 + 중급2 7강 = AI엔지니어링 코어 27강 친화 완성.**

---

## [4.19.0] - 2026-06-02

AI 엔지니어링 **중급1 "지식과 컨텍스트" 8강 전체를 친화 버전으로 완성**.

### Added

- **친화 중급1 8강 신설** (`friendly-intermediate1/01~08`) — "지식 트윈(AI 옆에 붙이는
  최신 자료 가방)" 척추 개념을 비개발자용 카드·비유·🎬사례·즉시실행 ai-try 박스·
  객관식 퀴즈로 재구성:
  - 01 AI에게 자료 가방 붙이기(학습 vs 트윈) / 02 한정된 책상에 자료 잘 놓기(컨텍스트)
  - 03 자료 가방 저장(영속) / 04 자료를 잘 잘라 담기(청킹·overlap·메타)
  - 05 정확히 찾아 다시 줄 세우기(하이브리드·재랭킹) / 06 출처 달기 & 모르면 모른다(인용·거부)
  - 07 대화로 잇고 최신 유지(독립 질의·갱신) / 08 전부 합쳐 나만의 지식 비서(Capstone)

### Changed

- **중급1(intermediate1) 트랙 01~08 전부 친화 버전으로 대체** — `intermediate1/index.ts`
  가 `asIntermediate1()` 매핑(기존 id·order·PRO 게이팅 유지). 코드판 `NN-*.ts` 는 파일로
  보존(번들 제외). **이제 입문 12강 + 중급1 8강 = 친화 버전 완성.**

---

## [4.18.0] - 2026-06-02

AI 엔지니어링 **입문(beginner) 트랙 12강 전체를 친화 버전으로 완성** — 07~12강 신설 +
입문 트랙 전 슬롯 교체.

### Added

- **친화 07~12강 신설** (`friendly-preview/07~12`) — 비개발자용 카드·비유·🎬사례·즉시실행
  ai-try 박스·객관식 퀴즈 형식:
  - 07 여러 AI가 팀으로 일하기(멀티 에이전트) — 조사·집필·검토 분담, 연쇄 실수 방어
  - 08 의미를 숫자로(임베딩) — 글을 의미 좌표로, 닮은 정도 계산
  - 09 내 문서로 답하는 AI(RAG) — 오픈북 시험 비유, 준 문서만 근거로 답
  - 10 검색을 더 똑똑하게(Hybrid) — 의미+키워드 결합, AI 재정렬
  - 11 AI 골라 쓰기(모델 비교) — 속도·품질·비용·프라이버시로 선택
  - 12 전부 합치기(Capstone) — 부품 조합으로 문서 Q&A 비서 설계
  - 각 강의 🎬 사례에 사례 미리채운 ai-try 실행 박스 포함.

### Changed

- **입문(beginner) 트랙 07~12강도 친화 버전으로 대체** — `beginner/index.ts` 가 01~12
  전부 `asBeginner()` 매핑(기존 id·order·PRO 게이팅 유지). 코드판 `07~12*.ts` 는 파일로
  보존(번들 제외). **이제 입문 트랙 12강 전체가 일반 사용자 친화 버전.**

---

## [4.17.1] - 2026-06-02

친화 **06강(스스로 일하는 AI — 에이전트)** 신설 + 입문 트랙 06강 슬롯 교체.

### Added

- **친화 06강 "스스로 일하는 AI (에이전트)"** — 생각→행동→관찰 루프, 에이전트=역할+도구
  조합, 안전(되돌릴 수 없는 행동은 사람 승인) 을 비개발자용 카드·비유로 재구성.
  🎬 회의 안내 메일 에이전트 사례 + 즉시 실행 ai-try 박스 2개(계획 세우기 / 메일 초안).
  `friendly-preview/06-single-agent.ts` 신규.

### Changed

- **입문(beginner) 트랙 06강을 친화 버전으로 대체** — `beginner/index.ts` 에서 06 슬롯을
  친화 콘텐츠로 매핑(`ai-eng-06-single-agent` id·order 유지, PRO 게이팅 그대로). 코드판
  `06-single-agent.ts` 는 파일로 보존(번들 제외). 이제 입문 01~06 친화 / 07~12 코드판.

---

## [4.17.0] - 2026-06-02

AI 엔지니어링 **입문(beginner) 트랙 01~05강을 일반 사용자 친화 버전으로 교체** +
각 강의 사례에 **즉시 실행 가능한 LLM 박스** 추가.

### Changed

- **입문 트랙 01~05강 친화 버전으로 대체** — `beginner/index.ts` 에서 01~05 슬롯을
  친화-미리보기 콘텐츠로 매핑. 기존 id·order·track 을 그대로 덮어써 **URL·진도·
  무료/PRO 게이팅 유지** (`ai-eng-01/02` 무료, `03~05` PRO 그대로). 메뉴의 'AI
  엔지니어링 입문'에서 바로 친화 강의로 진입·테스트 가능.
  - 기존 코드 중심 강의(`01-webllm-intro.ts`~`05-tool-calling.ts`)는 **파일로 보존**
    (import 만 분리) — 향후 "개발자 심화" 트랙에서 재사용. 06~12강은 코드판 유지.

### Added

- **사례 즉시 실행 LLM 박스** — 친화 01~05강의 🎬 샘플 사례 바로 아래에, 그 사례
  내용이 미리 채워진 `ai-try` 셀 추가. 학습자가 클릭 한 번으로 바로 실행해볼 수 있어
  교육 흐름이 빨라짐. (01강은 자유도 비교 포함 3개, 02~05강 각 2개의 LLM 박스)

### Fixed

- 코드 셀이 0개인 레슨(친화 콘텐츠)은 자동저장을 끈다 — 보존할 사용자 코드가 없어
  불필요한 저장 방지. `LessonPage` 의 `autoSaveEnabled` 에 `hasCodeCells` 조건 추가.

---

## [4.16.2] - 2026-06-02

친화 미리보기(friendly-preview) 트랙을 **전 강의 PRO 잠금**으로 통일.

### Changed

- **친화 강의 접근 정책 변경** — 기존엔 친화 01·02강이 무료(`FREE_AI_ENG_LESSON_IDS`)였으나,
  친화 트랙 전체(01~05강)를 **PRO 잠금**으로 변경. `content/tier.ts` 의 무료 목록에서
  `ai-eng-fr-01-ai-in-browser`·`ai-eng-fr-02-how-to-ask` 를 제거. 03·04·05강은 원래부터
  PRO. 이제 다섯 강의 모두 비-PRO/비-관리자 사용자에게 결제벽(LockedContentScreen) 적용.

---

## [4.16.1] - 2026-06-02

친화 미리보기 강의를 **같은 브라우저로 재방문하면 노트북이 빈 화면**으로 뜨던 버그 수정.

### Fixed

- **친화 강의 재방문 빈 화면** — 비로그인 방문자가 친화(`friendly-preview`) 강의를 두 번째
  로드하면 셀이 하나도 안 보이던 문제. 원인은 `authStore` 의 `onAuthStateChange` 가
  페이지 로드마다 발생시키는 `INITIAL_SESSION`(세션 없음=null) 이벤트에서 인메모리
  스토어를 `clearAllUserStores()` 로 비웠고, 그 `clear()` 가 `LessonPage` 의
  `loadCells()` 와 레이스를 일으켜 — 세션 조회가 캐시된 재방문(웜 로드)에서 셀을
  로드한 뒤 비워버린 것. **실제 로그아웃(`SIGNED_OUT`) 이벤트일 때만** 스토어를
  초기화하도록 게이트를 추가해 해결. 시크릿 창이 항상 정상이던 것도 동일 원인(세션
  캐시가 없어 항상 콜드 경로)으로 설명됨.

---

## [4.16.0] - 2026-06-01

AI 엔지니어링 트랙에 **중급2 "에이전틱" 7강** 신설 — 지식 트윈(중급1)에 손발을 달아
도구·오케스트레이션·HITL·평가·운영까지 다루는 프로덕션 에이전트 과정.

### Added

- **AI 엔지니어링 중급2 트랙 "에이전틱" (7강)** — 척추 개념: 트윈은 "안다", 에이전트는 "한다".
  - 01 견고한 단일 에이전트(에러·재시도·결과를 관측 가능한 값으로 반환·onStep 관측) /
    02 오케스트레이션(router·병렬·supervisor) / 03 메모리·컨텍스트(슬라이딩 윈도우·요약·장기 트윈) /
    04 실연동 + HITL 승인 게이트 + MCP 도구 소비(개념·의사코드) /
    05 평가·관측(평가셋 성공률·A/B·faithfulness LLM-as-judge·트레이싱) /
    06 비용·속도·보안(라우팅·캐싱 TTL·키 서버 프록시·프롬프트 인젝션 방어) /
    07 Capstone 배포형 에이전트(트윈 검색 읽기 도구 + 티켓 생성 쓰기 도구 HITL + 관측).
  - `chatWithTools(req, { maxIterations, onStep })` 루프, `responseSchema`(zod), `embed`/`VectorStore` 활용.
  - 입문/중급1 셀 사이클·용어 밑줄·hints·solution·퀴즈 포맷 준수.
- `content/index.ts` 에 `ai-engineering.intermediate2` 등록.

### Changed

- **트랙 파트 선택 페이지**(`/ai-dev/track`)에서 "중급2 — 에이전틱" 을 "준비 중" → **활성 링크**(7강, NEW)로 전환.

### Notes

- 중급2 레슨도 입문 Ch03~12·중급1 과 동일하게 **PRO 소프트 게이팅**(클라 번들 + UI 잠금). 서버 게이팅 미적용.
- 실제 외부 API·OAuth·MCP 연결·Vercel 배포는 브라우저 노트북 제약상 개념+의사코드로 다룸(HITL·캐시·인젝션은 셀에서 실동작).

---

## [4.15.0] - 2026-06-01

AI 엔지니어링 트랙을 **입문자/중급1/중급2 파트 구조**로 확장. 중급1 "지식과 컨텍스트" 8강 신설.

### Added

- **AI 엔지니어링 중급1 트랙 "지식과 컨텍스트" (8강)** — 척추 개념은 **런타임 지식 트윈**
  (학습된 가중치 ↔ 컨텍스트/RAG). RAG·Context 심화 과정.
  - 01 두 개의 뇌(학습 vs 트윈) / 02 컨텍스트 엔지니어링 / 03 영속 지식 베이스
    (직렬화 + localStorage) / 04 청킹·인덱싱(overlap + 메타데이터) /
    05 고급 검색·재랭킹(LLM 재랭킹 코드 — 입문 10강 미완 보강) /
    06 정확한 인용(번호↔청크 매핑 — 입문 12강 [출처 N] 부정확 해결) /
    07 멀티턴·지식 갱신(query rewriting) / 08 Capstone 도메인 지식 트윈 RAG.
  - 트랙 등록: `types/lesson.ts` Track 유니온에 `intermediate1`·`intermediate2`,
    `content/languages.ts` TRACKS, `content/index.ts` CURRICULA.
  - 입문 강의 셀 사이클·용어 밑줄·hints·solution·퀴즈 포맷 준수.
- **AI 엔지니어링 트랙 파트 선택 페이지** (`/ai-dev/track`, `AiEngTrackPage`) —
  입문자(12강)/중급1(8강, NEW)/MCP·중급2("준비 중") 파트를 고르는 화면.
- **버전 관리 문서** `MILESTONES.md` — 버전 규칙(SemVer) + 배포 전 체크리스트 + 마일스톤 로드맵.

### Changed

- `/ai-dev` "AI 엔지니어링 트랙" 카드가 곧장 입문 12강으로 가던 것을 **파트 선택 페이지**로 연결.
  히어로·로드맵 문구를 "입문+중급" 으로 갱신.
- **W00 워크샵(윈도우·맥)** 초보자용 설명 보강 + 글로서리 용어 추가.

### Fixed

- **AI 입문 트랙 교재 대조 정정** — Imagen 계열 표기·Sora 종료·카카오톡 원본(.heic) 등
  사실 정확화 (book2 대조 검토 반영).

### Notes

- 중급1 레슨은 입문 Ch03~12와 동일하게 **PRO 소프트 게이팅**(클라 번들 + UI 잠금).
  서버 게이팅(`api/*-content.ts` 패턴)은 미적용 — 후속 작업.
