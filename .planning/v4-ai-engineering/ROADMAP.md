...
**작성일**: 2026-04-15 (최종 갱신 2026-04-16)
**상태**: ✅ **Phase 1~5 전체 완료** — v4.6.0 12강 완성, UAT 대기
**목표 버전**: v4.0.0 ~ v4.6.0 (전체 출시 완료)
...
> **버전 슬롯 이동 주의** (2026-04-16 개정): Phase 1 완주 과정에서 v4.1·v4.2 를 UAT 핫픽스(키 UX, 녹화 인프라)에 사용. 이에 따라 후속 Phase 의 버전이 한 칸씩 뒤로 밀림.

### Phase 1 · **v4.0.0** ~ **v4.2.0** ✅ 완료 (2026-04-15~16)
**기반 + 입문**: 통합 SDK, Ch01 (WebLLM), Ch02 (Gemini)
- v4.0.0: Phase 1 코드 + 문서
- v4.0.1: 레슨 진입 크래시 핫픽스 + 진행률 바 + Ch01 1B 한계 프레이밍
- v4.1.0: 전역 KeySetupModal + missing-key 자동 오픈 + 로컬 진도 안내
- v4.1.1: Gemini 2.0→2.5 flash 모델 교체 (신규 사용자 차단 대응)
- v4.2.0: 강사용 녹화 다운로드 UI → 시뮬레이션 인프라 완성
- 성공 지표: ✅ 사용자가 Gemini 키 넣고 Ch02 셀 실행해 응답 받기 (+녹화본 생성)

### Phase 2 · **v4.3.0** ✅ 완료 (2026-04-16)
**텍스트 패턴 3종**: Ch03 구조화 출력, Ch04 CoT, Ch05 Tool Calling
- Thought Stream 셀 (🧠 UI), zod + responseSchema, chatWithTools
- PLAN: [v4.3/PLAN.md](./v4.3/PLAN.md)

### Phase 3 · **v4.4.0** ✅ 완료 (2026-04-16)
**에이전트 축**: Ch06 단일 에이전트, Ch07 멀티 에이전트
- AgentTraceViewer, agent-step OutputChunk
- PLAN: [v4.4/PLAN.md](./v4.4/PLAN.md)

### Phase 4 · **v4.5.0** ✅ 완료 (2026-04-16)
**지식 축**: Ch08 임베딩, Ch09 RAG 기초, Ch10 Hybrid RAG
- @huggingface/transformers, embed(), VectorStore, BM25, RRF
- PLAN: [v4.5/PLAN.md](./v4.5/PLAN.md)

### Phase 5 · **v4.6.0** ✅ 완료 (2026-04-16)
**통합·종합**: Ch11 생태계 비교, Ch12 종합 프로젝트
- 12강 전 기법 조합 AI 문서 Q&A 시스템
...
├── v4.3/
│   └── PLAN.md                  # Phase 2 상세 설계 📝
├── v4.4/                        # (Phase 3 시작 시 작성)
├── v4.5/                        # (Phase 4 시작 시 작성)
├── v4.6/                        # (Phase 5 시작 시 작성)
└── shared/
...
## ✅ Phase 1 착수 체크리스트 (모두 완료)

- [x] ROADMAP 작성
- [x] Phase 1 PLAN.md 작성
- [x] 통합 SDK 폴더 구조 설계
- [x] Gemini / Groq SDK 브라우저 호환 확인
- [x] WebLLM 초기 로딩 UX 설계
- [x] API 키 암호화 저장 전략 확정 (AES-GCM non-extractable)
- [x] 시뮬레이션 녹화 포맷 v1 정의
- [x] 강사용 녹화 다운로드 UI (v4.2.0)

## 🚀 Phase 2 착수 체크리스트

- [x] Phase 2 PLAN.md 작성
- [ ] `@google/genai` streaming / function declaration API 재확인
- [ ] `groq-sdk` stream + tool_calls 응답 shape 확인
- [ ] zod 버전 결정 (v3.25 vs v4)
- [ ] Thought Stream UX 디자인 확정
- [ ] 녹화 포맷 v2 스키마 확정 (토큰 타이밍 + tool steps)
- [ ] T1 타입 확장부터 착수
